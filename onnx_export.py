...
from models.model_atan import MPNet
...
def inference(a):
    ...
    with torch.no_grad():
        for index in track(test_indexes):
            ...
            output_names = ['amp_g', 'pha_g', 'com_g']
            dynamic_axes = {
                'noisy_amp' : {2: 'seq_len'},
                'noisy_pha': {2: 'seq_len'},
                'amp_g': {2: 'seq_len'},
            ...
            torch.onnx.export(model.cpu(), x, 'mpsenet.onnx', input_names=input_names, output_names=output_names, verbose='True', dynamic_axes=dynamic_axes, opset_version=12)
            break
